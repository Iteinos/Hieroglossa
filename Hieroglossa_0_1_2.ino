...
void ECDH_initiate_exchange(uint8_t *public_key_p, uint8_t *recipient_public_key_p)
{
  ...
  String ECDH_hex = ByteArrayToStringHex(public_key_p, 16);
...
void encryption_test(void *parameters)
{
  ...
  while (1)
  {
    esp_task_wdt_feed();
    total++;
    if (decrypt(encrypt(str)) == str)
      success++;
    Serial.println("Successful: " + String(success) + ", total: " + String(total) + ". Success rate: " + String(100 * (success / total)) + "%");
  }
}
